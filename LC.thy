...
(* binder_datatype 'a terms =
  Var 'a
| App "'a terms" "'a terms"
| Abs x::'a t::"'a terms" binds x in t
*)
...
lemma Abs_set3: "terms_ctor v = Abs x e \<Longrightarrow> \<exists>x' e'. terms_ctor v = Abs x' e' \<and> x' \<in> set2_terms_pre v \<and> e' \<in> set3_terms_pre v"
...
lemma Abs_avoid: "|A::'a::var_terms_pre set| <o |UNIV::'a set| \<Longrightarrow> \<exists>x' e'. Abs x e = Abs x' e' \<and> x' \<notin> A"
  apply (drule terms.TT_fresh_nchotomys[of _ "Abs x e"])
  apply (erule exE)
  apply (erule conjE)
   apply (drule sym)
...
inductive step :: "'a::var_terms_pre terms \<times> 'a terms \<Rightarrow> bool" (*(infixr "\<^bold>\<longrightarrow>" 25) *) where
...
type_synonym 'a T = "'a terms \<times> 'a terms"
type_synonym 'a V = "'a \<times> 'a"

definition "small (A::'a set) \<equiv> |A| <o |UNIV::'a set|" 
definition "ssbij f \<equiv> bij f \<and> small (supp (f::'a::var_terms_pre \<Rightarrow> 'a))" 
