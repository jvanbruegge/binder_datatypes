...
imports BSmall Supervariables BSmall (* ILC_good *)
begin
...
(* 
consts good :: "itrm \<Rightarrow> bool"

axiomatization where good_iVar: "\<And>x. good (iVar x) \<Longrightarrow> \<exists>xs. super xs \<and> x \<in> dsset xs"
axiomatization where good_iApp: "\<And>e1 es2. good (iApp e1 es2) \<Longrightarrow> good e1 \<and> (\<forall>e2\<in>sset es2. good e2)"
axiomatization where good_iLam: "\<And>xs e. good (iLam xs e) \<Longrightarrow> super xs \<Longrightarrow> good e"
axiomatization where good_irrename: 
"\<And>f e. bij f \<Longrightarrow> |supp f| <o |UNIV::ivar set| \<Longrightarrow> presSuper f \<Longrightarrow> good e \<Longrightarrow> good (irrename f e)" 
axiomatization where good_touchedSuperT: "\<And>e. good e \<Longrightarrow> finite (touchedSuperT e)"
*)


lemma bsmall_FFVars_irrename: 
"bij f \<Longrightarrow> |supp f| <o |UNIV::ivar set| \<Longrightarrow> presSuper f \<Longrightarrow> bsmall (supp f) \<Longrightarrow> bsmall (FFVars e) \<Longrightarrow> 
   bsmall (FFVars (irrename f e))"
 using bsmall_def touchedSuperT_def touchedSuperT_irrename by force

lemma bsmal_iApp_FFVars_imp: 
"bsmall (FFVars (iApp e1 es2)) \<Longrightarrow> bsmall (FFVars e1) \<and> (\<forall>e2\<in>sset es2. bsmall (FFVars e2))"
unfolding bsmall_def touchedSuper_iApp apply safe 
  using rev_finite_subset touchedSuper_mono apply fastforce
  by (metis UN_absorb finite_Un touchedSuperT_def touchedSuper_iApp)

lemma bsmal_iLam_FFVars_imp: 
"bsmall (FFVars (iLam xs e)) \<Longrightarrow> super xs \<Longrightarrow> bsmall (FFVars e)"
unfolding bsmall_def touchedSuper_iLam  
  using touchedSuperT_def touchedSuper_iLam by auto
...
"morFromTrm H \<equiv> 
 (\<forall>e. bsmall (FFVars e) \<longrightarrow> H e \<in> B) \<and>  
 (\<forall>x. \<comment> \<open> super xs \<and> x \<in> dsset xs \<longrightarrow>  \<close> H (iVar x) = iVarB x) \<and> 
 (\<forall>e1 es2. bsmall (FFVars (iApp e1 es2))
    \<longrightarrow> H (iApp e1 es2) = iAppB (H e1) (smap H es2)) \<and> 
 (\<forall>xs e. bsmall (FFVars (iLam xs e)) \<and> super xs \<longrightarrow> H (iLam xs e) = iLamB xs (H e)) \<and> 
 (\<forall>\<sigma> e. bsmall (FFVars e) \<and> bij \<sigma> \<and> |supp \<sigma>| <o |UNIV::ivar set| \<and> bsmall (supp \<sigma>) \<and> presSuper \<sigma>  
          \<longrightarrow> H (irrename \<sigma> e) = renB \<sigma> (H e)) \<and> 
 (\<forall>e. bsmall (FFVars e) \<longrightarrow> touchedSuper (FVarsB (H e)) \<subseteq> touchedSuper (FFVars e))"
...
inductive R where 
iVar[simp,intro]: "\<comment> \<open> super xs \<Longrightarrow> x \<in> dsset xs \<Longrightarrow>  \<close> R (iVar x) (iVarB x)"
|
iApp: "R e1 b1 \<Longrightarrow> 
   bsmall (FFVars (iApp e1 es2)) \<Longrightarrow> 
   stream_all2 R es2 bs2 \<Longrightarrow> 
   R (iApp e1 es2) (iAppB b1 bs2)"
|
iLam: "R e b \<Longrightarrow> super xs \<Longrightarrow> bsmall (FFVars (iLam xs e)) \<Longrightarrow> R (iLam xs e) (iLamB xs b)"
...
shows "\<exists>b1 bs2. R e1 b1 \<and> bsmall (FFVars (iApp e1 es2)) \<and> 
               stream_all2 R es2 bs2 \<and> b = iAppB b1 bs2"
using assms by (cases rule: R.cases) auto

lemma R_iLam_elim: 
assumes "R (iLam xs e) b"
shows "\<exists>xs' e' b'. super xs' \<and> R e' b' \<and> bsmall (FFVars (iLam xs e)) \<and> iLam xs e = iLam xs' e' \<and> b = iLamB xs' b'"
using assms by (cases rule: R.cases) auto

lemma R_total: 
"bsmall (FFVars e) \<Longrightarrow> \<exists>b. b \<in> B \<and> R e b"
proof(induct e)
  case (iVar x)
  show ?case apply(rule exI[of _ "iVarB x"]) by auto
next
  case (iApp e1 es2)
  moreover have bsm: "bsmall (FFVars e1)" "\<forall>e2\<in>sset es2. bsmall (FFVars e2)"  
    using bsmal_iApp_FFVars_imp iApp.prems by auto
  ultimately obtain b1 where b1: "b1 \<in> B" "R e1 b1" by auto
  from iApp(2) obtain E2 where 
  E2: "\<And>e2. e2 \<in> sset es2 \<Longrightarrow> bsmall (ILC.FFVars e2) \<Longrightarrow> (E2 e2) \<in> B \<and> R e2 (E2 e2)" by metis

  show ?case apply(rule exI[of _ "iAppB b1 (smap E2 es2)"], intro conjI)
    subgoal apply(rule iAppB_B) using bsm b1 E2 by auto
    subgoal apply(rule R.iApp) using bsm b1 E2 iApp(3) unfolding stream_all2_iff_snth by auto .   
next
  case (iLam xs e)
  moreover have "bsmall (ILC.FFVars e)" sledgehammer
