...
apply (metis Reneqv.GG_mmono2 Reneqv.II.simps predicate1I)
  subgoal premises p
    apply (insert p(3))
    apply (unfold G_def ex_simps conj_disj_distribL ex_disj_distrib Tperm_def)
  ...
  apply(intro conjI) 
    ...
    apply(cases t) unfolding presSuper_def apply simp apply(intro conjI)
      ...
      subgoal apply(subst irrename_eq_itvsubst_iVar)
        ...
        subgoal unfolding isPerm_def by auto
        subgoal unfolding id_on_def isPerm_def presBnd_def irrename_eq_itvsubst_iVar[symmetric]
          using p(2)[unfolded Tperm_def isPerm_def presBnd_def]
          by (auto split: option.splits) .
      by (smt (verit) Diff_iff Tsupp.simps Un_iff iLam_irrename id_on_def iterm.set(3) surjective_pairing) . .
  (* *)
