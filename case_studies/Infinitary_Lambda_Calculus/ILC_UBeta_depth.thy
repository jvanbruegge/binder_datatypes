...
unfolding fun_eq_iff G_def ex_simps conj_disj_distribL ex_disj_distrib snd_conv fst_conv apply clarify
  subgoal for R d tt1 tt2 apply(rule iffI)
    subgoal apply(elim disj_forward exE conjE)
      by auto
...
apply (metis Ustep.GG_mmono2 Ustep.II.simps predicate1I)
  subgoal premises p
    apply (insert p(3))
    unfolding ex_simps conj_disj_distribL ex_disj_distrib G_def
  ...
  subgoal
    ...
    apply(cases t) unfolding presSuper_def apply simp apply(intro conjI)
      ...
      subgoal unfolding stream.map_comp using p(2)[unfolded isPerm_def presBnd_def Tperm_def]
        apply -
        apply (rotate_tac -1)
        apply (erule allE)
        apply (rotate_tac -1)
        ...
        apply (erule impE)
         apply (rule conjI[rotated])+
          ...
          prefer 3
          apply simp
        unfolding presSuper_def[symmetric]
         apply (metis ILC_UBeta_depth.presBnd_def presBnd_presSuper)
        by blast
      apply(subst stream.map_comp)
      apply simp
      apply (rule stream.map_cong0)
        apply(subst iLam_irrename[of "f"]) unfolding id_on_def
