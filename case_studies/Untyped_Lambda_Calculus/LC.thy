theory LC
  ...
  "Case_Studies.Swapping_vs_Permutation"
  "Case_Studies.General_Customization"
...
locale xHL_REC_term =
  ...
  assumes
    ...
    and
    Pmap_cong_id: "\<And>d f. validP d \<Longrightarrow>
           bij f \<Longrightarrow>
           |supp f| <o |UNIV :: 'a set| \<Longrightarrow>
           (\<And>a. a \<in> PFVars d \<Longrightarrow> f a = a) \<Longrightarrow>
           Pmap f d = d" and
    PFVars_Pmap: "\<And>d f. validP d \<Longrightarrow>
           bij f \<Longrightarrow>
           |supp f| <o |UNIV :: 'a set| \<Longrightarrow>
           PFVars (Pmap f d) = f ` PFVars d" 
    and
    card_of_PFVars: "validP p \<Longrightarrow> |PFVars p| <o |UNIV :: 'a set|" 
    and
    validP_Pmap: "validP p \<Longrightarrow>
           bij f \<Longrightarrow>
           |supp f| <o |UNIV :: 'a set| \<Longrightarrow>
           validP (Pmap f p)" 
    and
    card_of_avoiding_set: "|avoiding_set| <o |UNIV :: 'a set|" 
    and
    Umap_comp: "\<And>d f g t.
    ...
    Umap_cong_id: "\<And>d t f.
       ...
       Umap f t d = d" 
    and
    validU_Umap: "\<And>f t u.
       validU u \<Longrightarrow>
       bij f \<Longrightarrow>
       |supp f| <o |UNIV :: 'a set| \<Longrightarrow>
       validU (Umap f t u)" 
    and
    Umap_UVar: "\<And>p x f. validP p \<Longrightarrow>
    bij f \<Longrightarrow>
    |supp f| <o |UNIV :: 'a set| \<Longrightarrow>
    imsupp f \<inter> avoiding_set = {} \<Longrightarrow>
    Umap f (Var x) (UVar x p) = UVar (f x) (Pmap f p)" 
    and
    Umap_ULam: "\<And>p x t pu f. validP p \<Longrightarrow>
    bij f \<Longrightarrow>
    |supp f| <o |UNIV :: 'a set| \<Longrightarrow>
    imsupp f \<inter> avoiding_set = {} \<Longrightarrow>
    ...
    and
    Umap_UApp: "\<And>p t1 pu1 t2 pu2 f. validP p \<Longrightarrow>
    bij f \<Longrightarrow>
    |supp f| <o |UNIV :: 'a set| \<Longrightarrow>
    imsupp f \<inter> avoiding_set = {} \<Longrightarrow>
    ...
    and
    UFVars_UVar: "\<And>p x. validP p \<Longrightarrow>
    UFVars (Var x) (UVar x p) \<subseteq> FFVars (Var x) \<union> PFVars p \<union> avoiding_set"
    and
    UFVars_ULam: "\<And>p x t pu. validP p \<Longrightarrow>
    pred_fun validP validU pu \<Longrightarrow>
    ...
    and
    UFVars_UApp: "\<And>p t1 pu1 t2 pu2. validP p \<Longrightarrow>
    pred_fun validP validU pu1 \<Longrightarrow>
    pred_fun validP validU pu2 \<Longrightarrow>
...
interpretation HL_to_LL: REC_term Pmap PFVars validP avoiding_set Umap UFVars Uctor validU
  ...
  subgoal for f y p
    ...
    apply (cases "Rep_term_pre y")
    subgoal for x
      apply (simp only: sum.case map_sum.simps id_apply fst_conv snd_conv map_prod_simp prod.case
    ...
    subgoal for x
      apply (cases x)
      subgoal for x'
        apply (simp only: map_sum.simps sum_set_simps sum.case
 Int_Un_distrib Int_Un_distrib2 Union_insert Union_empty Int_empty_left Un_empty_left Ball_def empty_iff simp_thms Un_empty_right prod.set_map UN_singleton
          Abs_term_pre_inverse[OF UNIV_I] App_def[symmetric] id_apply fst_conv snd_conv map_prod_simp prod.case)
        apply (cases x')
        subgoal for a b c
          apply (cases a)
          subgoal for a' a''
            apply (simp only:fsts.simps Un_iff snds.simps prod.case fst_conv snd_conv imp_disjL simp_thms all_conj_distrib map_prod_simp id_apply
      ...
      subgoal for x'
        apply (simp only: map_sum.simps sum_set_simps sum.case
 Int_Un_distrib Int_Un_distrib2 Union_insert Union_empty Int_empty_left Un_empty_left Ball_def empty_iff simp_thms Un_empty_right prod.set_map UN_singleton
          Abs_term_pre_inverse[OF UNIV_I] Lam_def[symmetric] id_apply fst_conv snd_conv map_prod_simp prod.case)
        apply (cases x')
        subgoal for a b c
          apply (simp only:fsts.simps Un_iff snds.simps prod.case fst_conv snd_conv imp_disjL simp_thms all_conj_distrib map_prod_simp id_apply
  ...
  subgoal for y p
    ...
    apply (tactic\<open>resolve_tac @{context} [BNF_FP_Util.mk_absumprodE @{thm type_definition_term_pre} [1,2,2] |> infer_instantiate' @{context} [SOME (Thm.cterm_of @{context} @{term y})]] 1\<close>)
      apply (simp only: sum.case map_sum.simps id_apply fst_conv snd_conv map_prod_simp prod.case comp_def
        Abs_term_pre_inverse[OF UNIV_I] Var_def[symmetric]
    ...
    apply (erule thin_rl)
      ...
      apply (drule meta_mp)
       apply (assumption)
      apply (erule meta_mp)
      apply (simp only: prod.collapse simp_thms)
    apply (erule thin_rl)
      apply (drule meta_spec)+
      apply (drule meta_mp)
       apply (assumption)
     ...
     apply (simp only: prod.collapse simp_thms)
      apply (simp only: sum.case map_sum.simps id_apply fst_conv snd_conv map_prod_simp prod.case comp_def
        ...
        prod.set_map UN_singleton split_beta Un_iff prod_set_simps
              imp_disjL all_conj_distrib insert_iff)
  ...
  subgoal for y p
    ...
    apply (cases "Rep_term_pre y")
    subgoal for x
      apply (simp only: sum.case validU_UVar)
      done
    subgoal for x
      apply (cases x)
      subgoal for x'
 ...
 Int_Un_distrib Int_Un_distrib2 Union_insert Union_empty Int_empty_left Un_empty_left Ball_def empty_iff simp_thms Un_empty_right prod.set_map UN_singleton)
        ...
        subgoal for a b c
          apply (cases a)
          subgoal for a' a''
            apply (simp only:fsts.simps Un_iff snds.simps prod.case fst_conv snd_conv imp_disjL simp_thms all_conj_distrib)
            apply (erule conjE)
            apply(rule validU_UApp)
               apply assumption+
            done
          done
        done
...
lemma HL_REC_term_Lam: "validP p \<Longrightarrow> {x} \<inter> (PFVars p \<union> avoiding_set) = {} \<Longrightarrow>
  ...
  unfolding Uctor_def map_term_pre_def set1_term_pre_def set2_term_pre_def set3_term_pre_def
    ...
    apply assumption
   apply (rule refl)
  apply (simp only: sum.case prod.case fst_conv snd_conv id_def)
  done

lemma HL_REC_term_App: "validP p \<Longrightarrow>
  HL_REC_term (App t1 t2) p = UApp t1 (\<lambda>p. if validP p then HL_REC_term t1 p else undefined) t2 (\<lambda>p. if validP p then HL_REC_term t2 p else undefined) p"
...
lemma FFVars_tvsubst[simp]:
  ...
  apply (binder_induction t avoiding: "IImsupp \<sigma>" rule: term.strong_induct)
     apply (auto simp: IImsupp_def assms[unfolded SSupp_def] intro!: term.Un_bound UN_bound term.set_bd_UNIV)
   apply (fastforce simp add: SSupp_def)
  by (metis empty_iff insert_iff term.set(1))
...
lemmas tvsubst_VVr_func[simp] = term.Sb_Inj[THEN fun_cong, unfolded id_apply]

lemmas term.inject(3)[simp del]

lemmas tvsubst_cong = term.Sb_cong
...
"bij (\<sigma>::var\<Rightarrow>var) \<Longrightarrow> |supp \<sigma>| <o |UNIV:: var set| \<Longrightarrow>
 (\<And>a'. a' \<in>FFVars e - {a::var} \<Longrightarrow> \<sigma> a' = a') \<Longrightarrow> Lam a e = Lam (\<sigma> a) (rrename \<sigma> e)"
...
corollary SSupp_upd_VVr_bound[simp,intro!]: "|SSupp (Var(a:=(t::trm)))| <o |UNIV::var set|"
  apply (rule iffD2[OF SSupp_upd_bound])
  apply (rule SSupp_Inj_bound)
  done
...
shows "|IImsupp (tvsubst (\<sigma>::var\<Rightarrow>trm) o \<tau>)| <o |UNIV:: var set|"
  using SSupp_IImsupp_bound s(1,2) term.SSupp_Sb_bound by blast

lemmas SSupp_tvsubst_bound = term.SSupp_Sb_bound[rotated]
...
shows "|SSupp (rrename (\<sigma>::var\<Rightarrow>var) o \<tau>)| <o |UNIV:: var set|"
  by (metis s term.SSupp_map_bound term.vvsubst_permute)
...
using IImsupp_rrename_update_bound[OF assms]
  by (metis (mono_tags) finite_Un finite_iff_le_card_var)
...
shows "rrename \<sigma> = tvsubst (Var o \<sigma>)"
  using term.vvsubst_permute term.map_is_Sb assms by metis
...
proof-
  ...
  show ?thesis
  apply(induct e1 rule: term.fresh_induct[where A = "{x} \<union> FFVars e2 \<union> imsupp \<sigma>"])
...
proof -
  have 1: "|SSupp (rrename \<sigma> \<circ> (Var(x := e2)))| <o |UNIV::var set|"
    using SSupp_rrename_update_bound assms(1,2) by blast
  then have 2: "|SSupp (rrename \<sigma> \<circ> (Var(x := e2)) \<circ> inv \<sigma>)| <o |UNIV::var set|"
    by (metis (no_types, opaque_lifting) assms(1,2) comp_assoc supp_SSupp_Var_le supp_inv_bound term.SSupp_Sb_bound term.Sb_comp_Inj)
  ...
  next
    ...
    then show ?case apply auto
      by (metis (no_types, opaque_lifting) comp_apply fun_upd_apply inv_simp1 inv_simp2 rrename_eq_tvsubst_Var supp_SSupp_Var_le term.subst(1))
  qed auto
qed

lemmas [equiv] =
  term.tvsubst_permute[THEN fun_cong, unfolded comp_def]
...
proof-
  ...
  apply(induct e1 rule: term.fresh_induct[where A = "{x,xx} \<union> FFVars e2"])
...
lemma Lam_inject: "(Lam x e = Lam x' e') = (\<exists>f. bij f \<and> |supp (f::var \<Rightarrow> var)| <o |UNIV::var set|
  \<and> id_on (FFVars (Lam x e)) f \<and> f x = x' \<and> rrename f e = e')"
