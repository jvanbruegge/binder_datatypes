...
definition sameShape_list :: "'a list \<Rightarrow> 'a list \<Rightarrow> bool" where 
  "sameShape_list x x' \<equiv> list_all2 top x x'"

definition nonrep_list :: "'a list \<Rightarrow> bool" where 
  "nonrep_list x \<equiv> \<forall> x'. sameShape_list x x' \<longrightarrow> (\<exists> f. x' = map f x)"

lemma sameShape_list_alt: "list_all2 top xs xs' = (length xs = length xs')"
...
lemma "nonrep_list [1::nat, 1, 1] \<longrightarrow> False"
  apply (unfold nonrep_list_def sameShape_list_def sameShape_list_alt)
...
definition sameShape_prod_1 :: "('a \<times> 'b) \<Rightarrow> ('a \<times> 'b) \<Rightarrow> bool" where 
  "sameShape_prod_1 x x' \<equiv> rel_prod top (=) x x'"

definition nonrep_prod_1 :: "('a \<times> 'b) \<Rightarrow> bool" where 
  "nonrep_prod_1 x \<equiv> \<forall> x'. sameShape_prod_1 x x' \<longrightarrow> (\<exists> f. x' = map_prod f id x)"

lemma "nonrep_prod_1 (a, b)"
  apply (unfold nonrep_prod_1_def sameShape_prod_1_def)
  apply (auto)
  done

typedef 'a nrp_list = "{(xs :: 'a list). nonrep_list xs}"
  apply (rule exI[of _ "[]"])
  apply (auto simp add: nonrep_list_def sameShape_list_def)
  done
