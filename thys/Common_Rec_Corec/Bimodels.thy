...
and ctorPermM_Ector': "\<And>u. ctorPermM Ector' Eperm u"
and ctorVarsM_Ector': "\<And>u. ctorVarsM Ector' EVrs u" 
(* above just the standard model properties; 
next some more specific requirements *)
and Ector_base_inj: "\<And>u1 u2::('a,'a,'a E, 'a E)G. base u1 \<Longrightarrow> Ector u1 = Ector u2 \<Longrightarrow> u1 = u2"
...
compatibility hypothesis (for step only since for base it 
is vacuous); the general one is inferable: *)
...
(* Alternatively, could have assumed some 
base-sensitive axioms, with simplified conditions for 
the base case 
taking advantage of the fact that base ensures no recursive components:

ctorPermM_base_Ector': "\<And>u. ctorPermM_base Ector' Eperm base u"
...
and then could have proved the standard model prperties from these: 
...
lemma ctorVarsM_Ector': "ctorVarsM Ector' EVrs u"
