...
corollary strong_induct_good[consumes 2, case_names iVriLm iAp]: 
...
lemma touchedSuperT_itvsubst: "|SSupp f| <o |UNIV :: ivar set| \<Longrightarrow> touchedSuperT (itvsubst f t) = \<Union> ((touchedSuperT o f) ` (iFV t))"
unfolding touchedSuperT_def by (auto simp: touchedSuper_UN )

lemma good_iFV_RSuper: "good e \<Longrightarrow> iFV e \<subseteq> RSuper"
apply(induct rule: good.induct) unfolding RSuper_def 
  by auto (metis superOf_subOf)

lemma good_iFV_super: "good e \<Longrightarrow> x \<in> iFV e \<Longrightarrow> \<exists>xs. super xs \<and> x \<in> dsset xs"
apply(drule good_iFV_RSuper) unfolding RSuper_def2 by auto
  ...
  then show ?case apply(subst iterm.subst)
      ...
      subgoal apply(rule good.iAp)
        ...
        apply(rule UN_touchedSuperT_super_eq)
          ...
          subgoal using good_iFV_RSuper by presburger
          subgoal using good_iFV_RSuper by presburger
...
e \<in> sset es \<Longrightarrow> touchedSuper (\<Union> (iFV ` (sset es))) = touchedSuper (iFV e)"
