...
next
  ...
  then show ?case unfolding affine_iApp_iff using istep_iFV by fastforce
next
  case (iAppR e1 es2 i e2')
  then show ?case unfolding affine_iApp_iff using istep_iFV
...
using assms apply(induct rule: istep.induct)
  subgoal for xs e1 es2 apply(rule imkSubst_affine_strong) 
    ...
    subgoal using affine_iApp_iff by auto .
  subgoal unfolding affine_iApp_iff using istep_iFV by fastforce
    subgoal unfolding affine_iApp_iff using istep_iFV 
    by simp (smt (verit, ccfv_SIG) Int_subset_empty1 More_Stream.theN disjoint_iff snth_sset snth_supd_diff snth_supd_same)
    subgoal by auto . 


definition "affineS es \<equiv> (\<forall>e\<in>sset es. affine e) \<and> (\<forall>i j. i \<noteq> j \<longrightarrow> ILC.iFV (snth es i) \<inter> ILC.iFV (snth es j) = {})"
...
by auto (metis disjoint_iff_not_equal hred_def insert_absorb insert_subset istep.Beta istep_iFV)


lemma affineS_sflat: "affineS (sflat ess) \<longleftrightarrow> 
 (\<forall>i j i' j'. affine (snth2 ess (i,j)) \<and> 
    ((i,j) \<noteq> (i',j') \<longrightarrow> ILC.iFV ((snth2 ess (i,j))) \<inter> ILC.iFV ((snth2 ess (i',j'))) = {}))"
...
"affineS (smap2 iAp es ess) \<longleftrightarrow> 
 affineS es \<and> affineS (sflat ess) \<and> 
  (\<forall>i j k. ILC.iFV (snth es i) \<inter> ILC.iFV (snth2 ess (j,k)) = {})"
...
lemma affineS_smap_iLm_iff: "affineS (smap (iLm xs) es) \<longleftrightarrow> 
  (\<forall>i j. i \<noteq> j \<longrightarrow> affine (snth es i) \<and> ILC.iFV (snth es i) \<inter> ILC.iFV (snth es j) \<subseteq> dsset xs)"
...
using assms apply(induct rule: ustepD.induct)
  ...
  subgoal apply auto unfolding affine_iApp_iff using ustepD_iFV by fastforce
  subgoal apply auto unfolding affine_iApp_iff using ustepD_iFV 
  ...
  subgoal for es es' xs apply(frule ustepD_iFV) by auto  . 

lemma ustepD_affineS:
assumes "ustepD d es es'" and "affineS es"
shows "affineS es'"
using assms ustepD_affine ustepD_iFV unfolding affineS_def apply auto 
