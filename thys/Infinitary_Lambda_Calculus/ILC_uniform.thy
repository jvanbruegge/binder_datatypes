...
shows "\<exists>f. bij f \<and> |supp f| <o |UNIV::ivar set| \<and> presSuper f \<and> finite (touchedSuper (supp f)) \<and>
       id_on (ILC.FFVars (iLm xs e)) f \<and> id_on (- (dsset xs \<union> dsset xs')) f \<and> 
...
proof-
  ...
  "id_on (FFVars (iLm xsa ea)) f" "id_on (- (dsset xsa \<union> dsset xs)) f" 
  ...
  have "FFVars (iLm xs e) \<inter> dsset xs = {}" by auto
  ...
  show ?thesis apply(rule exI[of _ "irrename f ea'"]) apply(intro conjI)
    subgoal unfolding ee' unfolding iLm_inject apply(rule exI[of _ f]) apply(intro conjI)
      subgoal by fact
      subgoal by fact
...
lemma reneqv_iAp_casesR:
assumes rr: "reneqv ee (iAp e1' es2')"
shows "\<exists> e1 es2. ee = iAp e1 es2 \<and> reneqv e1 e1' \<and> 
          (\<forall>e e'. {e,e'} \<subseteq> sset es2 \<union> sset es2' \<longrightarrow> reneqv e e')"
using assms by cases auto

lemma reneqv_iAp_iff:
...
next
  ...
  using reneqv_iAp_casesL[OF iAp(4)] by auto
...
proof-
  ...
  define B where B: "B = A \<union> dsset xs' \<union> FFVars (iLm xs e)"
...
proof- 
  ...
  using u uniform_iAp_case by blast
  have u': "uniform (iLm xs e1)" using u uniform_iAp_iff by blast

  have fves2: "touchedSuper (\<Union> (FFVars ` (sset es2))) = touchedSuper (FFVars e2) "  
    by (metis e2 iAp_inject reneqvS_def reneqv_iAp_casesL u uniformS_def3 uniformS_touchedSuper uniform_def3)
  ...
  "dsmap f xs = xs'" "irrename f e1 = e1'" using il[unfolded iLm_inject] by auto
...
uniformS_def4 uniformS_sset_uniform uniform_finite_touchedUponT uniform_iAp_iff)
