...
theory Swapping_vs_Permutation 
  imports (* SwapFreeV_model *) FixedCountableVars
begin 
...
definition finite_supp :: "('c \<Rightarrow> var \<Rightarrow> var \<Rightarrow> 'c) \<Rightarrow> 'c \<Rightarrow> bool" where
  "finite_supp swp c \<equiv>
  \<exists> A. finite A \<and> supports swp A c"

(* Set-based version of the axioms: *)
definition nswappingOn :: "'a set \<Rightarrow> ('a \<Rightarrow> var \<Rightarrow> var \<Rightarrow> 'a) \<Rightarrow> bool" where 
...
fun compose :: "(var \<times> var) list \<Rightarrow> (var \<Rightarrow> var)" where 
 "compose [] = id"
...
fun act :: "('c \<Rightarrow> var \<Rightarrow> var \<Rightarrow> 'c) \<Rightarrow> (var \<times> var) list \<Rightarrow> 'c \<Rightarrow> 'c" where 
 "act swp [] c = c"
...
lemma compose_support_im:"compose x1x2s x \<noteq> x \<Longrightarrow> 
  ...
  using compose_support0 compose_diff_implies_diff_im by blast
...
next
  ...
  proof (cases "x0 = x2")
    ...
    obtain x00 where x00:"x00 \<notin> {x1,x2}" 
        using exists_var by (meson finite.emptyI finite.insertI)
...
lemma compose_push_left: assumes notem:"x1x2s \<noteq> []" 
...
lemma act_compose_push_left: assumes notem:"x1x2s \<noteq> []" 
  and swp:"nswapping swp"
  and inset:"\<exists> p. p \<in> set x1x2s \<and> (x2 = fst p \<or> x2 = snd p)"
  and image:"compose x1x2s x2 = x1"
shows "\<exists> x1x2s'. length x1x2s' = length x1x2s 
...
shows "\<exists> z1z2s. \<sigma> = compose z1z2s"
  using assms
...
lemma compose_asTrans: "fsupp \<sigma> \<Longrightarrow> bij \<sigma> \<Longrightarrow> compose (asTrans \<sigma>) = \<sigma>"
  by (metis (full_types) asTrans_def fsupp_ex_compose someI_ex)
