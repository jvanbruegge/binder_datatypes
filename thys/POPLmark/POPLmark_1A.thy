...
lemma wf_concat: "\<lbrakk> wf \<Delta> ; wf \<Gamma> ; \<Gamma> \<bottom> \<Delta> \<rbrakk> \<Longrightarrow> wf (\<Gamma>,,\<Delta>)"
proof (induction \<Delta> rule: wf.induct)
  case (wf_Cons x \<Delta> T)
  then have 1: "(\<Gamma>,, (\<Delta> ,, x <: T)) = ((\<Gamma>,, \<Delta>),, x <: T)" by simp
  show ?case unfolding 1
    apply (rule wf.wf_Cons)
...
lemma ty_weakening:
  assumes "\<Gamma> \<turnstile> S <: T" "wf (\<Gamma>,,\<Delta>)"
...
next
  ...
  have 2: "wf (\<Gamma> ,, \<Delta> ,, x <: T\<^sub>1)"
...
lemma context_determ: "\<lbrakk> X <: U \<in> (\<Gamma> ,, X <: U',, \<Delta>) ;  wf (\<Gamma> ,, X <: U',, \<Delta>) \<rbrakk> \<Longrightarrow> U = U'"
proof (induction \<Delta>)
  case Nil
  then show ?case
...
proof (induction \<Delta>)
  ...
  have "b \<notin> dom (\<Gamma>,, X <: R,, \<Delta>)" using Cons(2)[unfolded 1] by auto
...
proof -
  ...
  proof (binder_induction Q arbitrary: \<Gamma> \<Delta> S T M N X R avoiding: X "dom \<Gamma>" "dom \<Delta>" rule: Type.strong_induct)
    ...
    } note ty_trans = this
    {
      ...
      next
        ...
        have 1: "wf (\<Gamma>,, X <: R,, \<Delta>',, Z <: T\<^sub>1)"
          apply (rule wf_Cons)
  ...
  next
    ...
    show ty_trans: "\<Gamma> \<turnstile> S <: Top \<Longrightarrow> \<Gamma> \<turnstile> Top <: T \<Longrightarrow> \<Gamma> \<turnstile> S <: T" by auto
    {
      case 2
      then show ?case
      proof (binder_induction "\<Gamma>,, X <: Top,, \<Delta>" M N arbitrary: \<Delta> avoiding: X "dom \<Gamma>" "dom \<Delta>" rule: ty.strong_induct)
        case (SA_Trans_TVar Z U T \<Delta>')
        then show ?case
        proof (cases "X = Z")
          case True
  ...
  next
    ...
    } note ty_trans = this
    {
      case 2
      then show ?case
      proof (binder_induction "\<Gamma>,, X <: (Q\<^sub>1 \<rightarrow> Q\<^sub>2),, \<Delta>" M N arbitrary: \<Delta> avoiding: X "dom \<Gamma>" "dom \<Delta>" rule: ty.strong_induct)
        case (SA_Trans_TVar Z U T \<Delta>')
        then show ?case
        proof (cases "X = Z")
          case True
  ...
  next
    ...
    next
      ...
      proof (binder_induction "\<Gamma>,, Y <: (\<forall>X<:Q\<^sub>1. Q\<^sub>2),, \<Delta>" M N arbitrary: \<Delta> avoiding: X Y "dom \<Gamma>" "dom \<Delta>" rule: ty.strong_induct)
        case (SA_Trans_TVar Z U T \<Delta>')
        then show ?case
        proof (cases "Y = Z")
          case True
      ...
      next
        case (SA_All T\<^sub>1 S\<^sub>1 Z S\<^sub>2 T\<^sub>2 \<Delta>')
        have 1: "wf (\<Gamma>,, Y <: R,, \<Delta>',, Z <: T\<^sub>1)"
          apply (rule wf_Cons)
          using SA_All UnI1 image_iff by auto
...
proof -
  have 1: "wf (\<Gamma>,, X <: R,, \<Delta>)" using assms(2-3) narrow_wf well_scoped(1) wf_context by blast
