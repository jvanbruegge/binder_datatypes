...
lemma SA_AllE1[consumes 2, case_names SA_Trans_TVar SA_All]:
  assumes "\<Gamma> \<turnstile> \<forall>X<:S\<^sub>1. S\<^sub>2 <: T" "X \<notin> dom \<Gamma>"
    ...
    and All: "\<And>\<Gamma> T\<^sub>1 T\<^sub>2. \<lbrakk> \<Gamma> \<turnstile> T\<^sub>1 <: S\<^sub>1 ; \<Gamma> \<^bold>, X<:T\<^sub>1 \<turnstile> S\<^sub>2 <: T\<^sub>2 \<rbrakk> \<Longrightarrow> R \<Gamma> (\<forall>X<:S\<^sub>1. S\<^sub>2) (\<forall>X<:T\<^sub>1 . T\<^sub>2)"
...
using assms(1,2) proof (binder_induction \<Gamma> "\<forall>X<:S\<^sub>1. S\<^sub>2" T avoiding: \<Gamma> "\<forall>X<:S\<^sub>1. S\<^sub>2" T rule: ty.strong_induct)
  ...
  have 1: "\<forall>Y<:T\<^sub>1 . T\<^sub>2 = \<forall>X<:T\<^sub>1. permute_type (X \<leftrightarrow> Y) T\<^sub>2"
    unfolding type.inject
    using SA_All(10,7) well_scoped(2) by (fastforce simp: swap_sym)
  have fresh: "X \<notin> TFV T\<^sub>1"
    by (meson SA_All(5,10) in_mono well_scoped(1))
  have same: "R\<^sub>1 = S\<^sub>1" using SA_All(9) type_inject by blast
  have x: "\<forall>Y<:S\<^sub>1. R\<^sub>2 = \<forall>X<:S\<^sub>1. permute_type (X \<leftrightarrow> Y) R\<^sub>2"
    unfolding type.inject
    apply (auto simp: swap_sym)
    using SA_All.hyps(9) type.inject(3) by blast
  show ?case unfolding 1
    apply (rule All)
...
lemma SA_AllE2[consumes 2, case_names SA_Trans_TVar SA_All]:
  assumes "\<Gamma> \<turnstile> S <: \<forall>X<:T\<^sub>1. T\<^sub>2" "X \<notin> dom \<Gamma>"
    ...
    and All: "\<And>\<Gamma> S\<^sub>1 S\<^sub>2. \<lbrakk> \<Gamma> \<turnstile> T\<^sub>1 <: S\<^sub>1 ; \<Gamma> \<^bold>, X<:T\<^sub>1 \<turnstile> S\<^sub>2 <: T\<^sub>2 \<rbrakk> \<Longrightarrow> R \<Gamma> (\<forall>X<:S\<^sub>1. S\<^sub>2) (\<forall> X <: T\<^sub>1 . T\<^sub>2)"
...
using assms(1,2) proof (binder_induction \<Gamma> S "\<forall>X<:T\<^sub>1. T\<^sub>2" avoiding: \<Gamma> S "\<forall>X<:T\<^sub>1. T\<^sub>2" rule: ty.strong_induct)
  ...
  have 1: "\<forall>Y<:S\<^sub>1. S\<^sub>2 = \<forall>X<:S\<^sub>1. permute_type (Y \<leftrightarrow> X) S\<^sub>2"
    unfolding type.inject
  ...
  have fresh2: "X \<notin> TFV T\<^sub>1" "Y \<notin> TFV T\<^sub>1"
     apply (metis SA_All(5,9) in_mono fresh(1) type_inject well_scoped(1))
    by (metis SA_All(5,9) in_mono fresh(2) type_inject well_scoped(1))
  have same: "R\<^sub>1 = T\<^sub>1" using SA_All(9) type_inject by blast
  have x: "\<forall>Y<:T\<^sub>1 . R\<^sub>2 = \<forall>X<:T\<^sub>1. permute_type (Y \<leftrightarrow> X) R\<^sub>2"
    unfolding type.inject apply auto
    using SA_All.hyps(9) type.inject(3) by blast
  show ?case unfolding 1
    apply (rule All)
     apply (metis SA_All(5,9) type_inject)
...
proof -
  ...
  proof (binder_induction Q arbitrary: \<Gamma> \<Delta> S T M N X R avoiding: X "dom \<Gamma>" "dom \<Delta>" rule: type.strong_induct)
    ...
    } note ty_trans = this
    {
      case 2
      then show ?case
      proof (binder_induction "\<Gamma> \<^bold>, X <: TVr Y\<^bold>, \<Delta>" M N arbitrary: \<Delta> avoiding: X "dom \<Gamma>" "dom \<Delta>" rule: ty.strong_induct)
        case (SA_Trans_TVar Z U T \<Delta>')
        show ?case
        proof (cases "X = Z")
          case True
          then have u: "U = TVr Y" using SA_Trans_TVar(1,2) context_determ wf_context by blast
          have "TVr Y closed_in \<Gamma> \<^bold>, Z <: R\<^bold>, \<Delta>'" using SA_Trans_TVar(2) True u well_scoped(1) by fastforce
          then have "\<Gamma> \<^bold>, Z <: R \<^bold>, \<Delta>' \<turnstile> TVr Y <: T" using SA_Trans_TVar True u by auto
          moreover have "\<Gamma> \<^bold>, Z <: R\<^bold>, \<Delta>' \<turnstile> R <: TVr Y" using ty_weakening[OF ty_weakening_extend[OF SA_Trans_TVar(4)]]
      ...
      next
        ...
        then show ?case
          by (smt (verit, del_insts) SA_TRecER ty.SA_TRec ty_refl type.distinct(4) well_scoped(2))
  ...
  next
    ...
    show ty_trans: "\<Gamma> \<turnstile> S <: Top \<Longrightarrow> \<Gamma> \<turnstile> Top <: T \<Longrightarrow> \<Gamma> \<turnstile> S <: T" by auto
    {
      ...
      next
        case (SA_TRec X Y \<Delta>')
        then show ?case
          by (smt (verit) SA_TRecER ty.SA_TRec ty_refl type.distinct(17) well_scoped(2))
      qed (rule context_set_bd_UNIV | blast)+
    }
  next
    case (Arr Q\<^sub>1 Q\<^sub>2 \<Gamma> \<Delta> S T M N X R)
    {
      fix \<Gamma> S T
      ...
      proof (induction \<Gamma> S "Q\<^sub>1 \<rightarrow> Q\<^sub>2" rule: ty.induct)
        ...
        next
          ...
          then show ?thesis using left by (metis Arr(1,3) SA_Arrow type.inject)
        qed auto
      qed auto
    } note ty_trans = this
    {
      ...
      next
        case (SA_TRec X Y \<Delta>')
        then show ?case
          by (smt (verit, ccfv_threshold) SA_TRecER ty.SA_TRec ty_refl type.distinct(17) well_scoped(2))
      qed (rule context_set_bd_UNIV | blast)+
    }
  next
    case (All X Q\<^sub>1 Q\<^sub>2 \<Gamma> \<Delta> S T M N Y R)
    ...
    proof -
      ...
      proof (induction rule: SA_AllE2)
        ...
        next
          ...
          have 1: "\<Gamma> \<turnstile> T\<^sub>1 <: S\<^sub>1" by (rule All(4)[OF SA_All(1,3)])
          have "\<Gamma> \<^bold>, X <: T\<^sub>1 \<turnstile> S\<^sub>2 <: Q\<^sub>2" using All(5)[of \<emptyset> X \<Gamma> S\<^sub>2 Q\<^sub>2, OF _ SA_All(1)] SA_All(4)
            by (metis (mono_tags, lifting) SA_All(2) append.left_neutral fst_conv image_insert list.simps(15) well_scoped(1) wf_context)
          then have "\<Gamma> \<^bold>, X <: T\<^sub>1 \<turnstile> S\<^sub>2 <: T\<^sub>2" by (rule All(6)[OF _ SA_All(2)])
    ...
    {
      case 1
      then show ?case using ty_trans All(2) by blast
    next
      case 2
      then show ?case using All(1-3)
...
next
  case (Arr x1 x2 \<Gamma> S T)
  from Arr(4,3) show ?case
    ...
    proof (induction \<Gamma> S "x1 \<rightarrow> x2")
      ...
      show ?case
        apply (rule ty.SA_Arrow)
         apply (metis Arr(1) SA_Arrow(1,5,6,8) type.inject(2))
        by (metis Arr(2) SA_Arrow(3,5,7,8) type.inject(2))
    qed auto
  qed auto
next
  case (All X Q\<^sub>1 Q\<^sub>2 \<Gamma> S T)
  from All(4,1,5,1) show ?case
  proof (induction rule: SA_AllE2)
    ...
    next
      ...
      then have "\<Gamma> \<^bold>, X <: T\<^sub>1 \<turnstile> S\<^sub>2 <: Q\<^sub>2"
        using ty_narrowing_aux[where \<Delta>="[]", unfolded append.simps] All(2) by blast
      then have "\<Gamma> \<^bold>, X <: T\<^sub>1 \<turnstile> S\<^sub>2 <: T\<^sub>2" using All(3) SA_All(2) by blast
      moreover have "\<Gamma> \<turnstile> T\<^sub>1 <: S\<^sub>1" by (rule All(2)[OF SA_All(1,3)])
