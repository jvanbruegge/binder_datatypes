...
"Tsupp e1 e2 \<equiv> FFVars e1 \<union> FFVars_comP e2"
...
ML \<open>fun gen_fresh ctxt xs0 acts0 ts0 = HEADGOAL (Subgoal.FOCUS_PARAMS (fn {context = ctxt, params = cps, ...} =>
  let
    ...
    (*val tss = map (mk ts0) [@{typ "var procP"}, @{typ "var comP"}];*)
...
lemma FFVars_comP_Cmt: "FFVars_comP (Cmt act P) = fvars act \<union> (FFVars P - bvars act)"
  by (cases act) auto
...
lemma Bout_inject: "(Bout x y t = Bout x' y' t') \<longleftrightarrow>
  ...
  \<and> id_on (FFVars_procP t - {y}) f \<and> f y = y' \<and> rrename_procP f t = t')"
...
lemma Cmt_rrename_bound_action: "bij (f :: var \<Rightarrow> var) \<Longrightarrow> |supp f| <o |UNIV :: var set| \<Longrightarrow> id_on (FFVars P - bvars \<alpha>) f \<Longrightarrow>
  ...
  by (cases \<alpha>)
    (force simp: Bout_inject id_on_def intro!: exI[of _ f] procP.rrename_cong_ids[symmetric] rrename_cong)+
